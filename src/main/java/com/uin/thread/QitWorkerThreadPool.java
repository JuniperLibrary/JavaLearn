...
@Slf4j
public class QitWorkerThreadPool<T> {

  private final int threadCount;
  private final ToIntFunction<T> hashFunction;
  private final List<LinkedBlockingQueue<T>> queues = new ArrayList<>();

  public QitWorkerThreadPool(String name, int threadCount, Consumer<T> consumer,
    ...
    for (int index = 0; index < threadCount; index++) {
      ...
      queues.add(queue);
      QitThreadFactory.startThread(String.format("%s-%d", name, index), () -> {
        while (true) {
          T obj = null;
          try {
            obj = queue.take();
